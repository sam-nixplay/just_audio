...
- (instancetype)initWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar playerId:(NSString*)idParam loadConfiguration:(NSDictionary *)loadConfiguration {
    ...
    _playerId = idParam;
    _methodChannel = [FlutterMethodChannel methodChannelWithName:[NSMutableString stringWithFormat:@"com.ryanheise.just_audio.methods.%@", _playerId] binaryMessenger:[registrar messenger]];
    _eventChannel = [[BetterEventChannel alloc] initWithName:[NSMutableString stringWithFormat:@"com.ryanheise.just_audio.events.%@", _playerId] messenger:[registrar messenger]];
    _dataEventChannel = [[BetterEventChannel alloc] initWithName:[NSMutableString stringWithFormat:@"com.ryanheise.just_audio.data.%@", _playerId] messenger:[registrar messenger]];
    _index = 0;
...
- (void)concatenatingRemoveRange:(NSString *)catId start:(int)start end:(int)end shuffleOrder:(NSArray<NSNumber *> *)shuffleOrder {
    NSMutableArray *matches = [[NSMutableArray alloc] init];
    [_audioSource findById:catId matches:matches];
    for (int i = 0; matches.count > 0 && i < matches.count; i++) {
        ConcatenatingAudioSource *catSource = (ConcatenatingAudioSource *)matches[i];
...
- (AudioSource *)decodeAudioSource:(NSDictionary *)data {
    ...
    if ([@"progressive" isEqualToString:type]) {
        ...
        return [[ConcatenatingAudioSource alloc] initWithId:data[@"id"] audioSources:[self decodeAudioSources:data[@"children"]] shuffleOrder:(NSArray<NSNumber *> *)data[@"shuffleOrder"] lazyLoading:(NSNumber *)data[@"useLazyPreparation"]];
    } else if ([@"clipping" isEqualToString:type]) {
        return [[ClippingAudioSource alloc] initWithId:data[@"id"] audioSource:(UriAudioSource *)[self decodeAudioSource:data[@"child"]] start:data[@"start"] end:data[@"end"]];
...
- (void)load:(NSDictionary *)source initialPosition:(CMTime)initialPosition initialIndex:(NSNumber *)initialIndex result:(FlutterResult)result {
    ...
    if (_audioSource && [@"clipping" isEqualToString:source[@"type"]]) {
        ...
        if (child && uri && [child.uri isEqualToString:uri]) {
            ClippingAudioSource *clipper = [[ClippingAudioSource alloc] initWithId:source[@"id"] audioSource:child start:source[@"start"] end:source[@"end"]];
    ...
    if (!_player) {
        ...
        if (@available(macOS 10.11, iOS 6.0, tvOS 9.0, *)) {
            _player.allowsExternalPlayback = _allowsExternalPlayback;
        }
        [_player addObserver:self forKeyPath:@"currentItem" options:NSKeyValueObservingOptionNew context:nil];
        __unsafe_unretained typeof(self) weakSelf = self;
        if (@available(macOS 10.12, iOS 10.0, tvOS 10.0, *)) {
            _timeObserver = [_player addPeriodicTimeObserverForInterval:CMTimeMake(200, 1000) queue:nil usingBlock:^(CMTime time) {
                [weakSelf checkForDiscontinuity];
            }];
        } else {
            _timeObserver = [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1000) queue:nil usingBlock:^(CMTime time) {
                [weakSelf checkForDiscontinuity];
            }];
        }
    }
    ...
    if (_indexedAudioSources.count == 0 || !_player.currentItem || _player.currentItem.status == AVPlayerItemStatusReadyToPlay) {
        _processingState = ready;
...
- (void)sendErrorForItem:(IndexedPlayerItem *)playerItem {
    FlutterError *flutterError = [FlutterError errorWithCode:[NSString stringWithFormat:@"%d", (int)playerItem.error.code] message:playerItem.error.localizedDescription details:@{@"index": @([self indexForItem:playerItem])}];
...
- (void)abortExistingConnection {
    FlutterError *flutterError = [FlutterError errorWithCode:@"abort" message:@"Connection aborted" details:nil];
